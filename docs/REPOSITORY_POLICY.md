...
- Immediately before merge, fetch `origin/main`, require it to be an ancestor of the exact closeout head, reread that unchanged remote branch and pull-request head, and verify the configured protection controls. A base advance requires exact provisional reversal, an ordinary history-preserving merge of the fetched base, and complete validation, exact-SHA review, closeout, non-force push/readback, and CI redelivery. Published task branches are never rebased or force-pushed.
...
- Completion requires fresh readback of the same merged PR, its exact reported merge OID reachable from fetched `origin/main`, tagged subject, exact first-parent archive introduction with active absence, synchronized clean local `main`, and absent remote task branch.
