...
Append-only archive of completed product/code implementation tasks.

Each archived task records the implementation authority that completed with `Status: passed` and `Pass: true`.

## Immutability Contract

- A task enters this file only through the closeout procedure in `.harness/validation.md`.
- Closeout preserves the task's semantic fields and changes only `Status` and `Pass`.
- After an archive entry exists in configured base-branch history, its meaning, identity, and order must not change.
...
- An archive entry on an unmerged task branch is provisional and may be reversed before merge when implementation repair or withdrawal requires it.
- A provisional archive entry does not satisfy completion, dependency, or queue-advancement proof.
- An entry becomes durable only when the configured procedure proves the merged pull request, tagged base-branch history, completed-store presence, active-store absence, and task-branch cleanup.

Git remains the detailed implementation and delivery history.

## Earlier Task Schema

`T-0001` through `T-0007` retain the task fields recorded when they ran, before the active/completed store split. Their merged tagged commits are the completion evidence. New tasks use the current semantic template in `.harness/tasks.md`.
