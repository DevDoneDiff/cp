...
AUTHORING_LANE_STATE: available
AUTHORING_BRANCH_PATTERN: codex/authoring-<slug>
AUTHORING_RECOVERY_RECORD: .harness/work/authoring-<slug>.md
...
EXACT_HEAD_CI_PROCEDURE: Require the configured-repository head and base identities, remote delivery-branch SHA, PR headRefOid, EXPECTED_HEAD_SHA, both exact required check results, and the post-query PR headRefOid readback to describe one unchanged task closeout or authoring candidate head; record the check identities and links
PRE_MERGE_BASE_REFRESH_PROCEDURE: Immediately before either manual or autonomous merge, run git fetch --prune origin without changing the working tree or current branch; record the exact fetched origin/BASE_BRANCH as EXPECTED_BASE_SHA; require it to be an ancestor of EXPECTED_HEAD_SHA, require the exact remote delivery branch and sole PR headRefOid still equal EXPECTED_HEAD_SHA, require atomic server-side up-to-date and no-bypass enforcement, and perform the applicable task-reversal or authoring base-merge redelivery if the fetched base advanced
...
AUTHORING_MERGE_COMMAND: gh pr merge <PR> --repo DevDoneDiff/cp --squash --delete-branch --match-head-commit <EXPECTED_HEAD_SHA> --subject "<DESCRIPTIVE_AUTHORING_TITLE>"
...
AUTHORING_MERGE_READBACK_PROCEDURE: Reuse the same exact PR, head, base, merge-OID, reachability, first-parent, branch-absence, and fast-forward readback; additionally require the descriptive untagged subject, merge tree equal to the reviewed authoring head tree, and .harness/completed.md byte-identical to its blob in EXPECTED_BASE_SHA
...
Operational availability requires `AUTHORING_LANE_STATE: available` and canonical completion proof for exact T-0036 on the fetched `BASE_BRANCH`. That same merged tree must contain this complete procedure and both producer-skill routes. A worktree, branch, candidate, or configuration token is never availability proof. Until the complete merged predicate passes, treat the lane as unavailable: do not push an authoring branch, create or update an authoring pull request, merge authoring work, or route an authoring skill through this lane.

## Non-Task Authoring Delivery

Use this procedure only after the operational-availability predicate above passes and explicit user instruction authorizes the authoring delivery. That instruction separately states whether guarded merge may be autonomous; task `RUN_MODE` and `MERGE_MODE` do not apply.

1. From a clean fetched `BASE_BRANCH`, inventory the intended authoring diff, all task stores and counters, local and remote task branches, live task pull requests, provisional closeout, and other live authoring identities. Classify whether the diff changes the active queue, counters, this registry, or task-execution authority in `AGENTS.md`; when it does, any live task identity blocks authoring mutation or merge. Create the one descriptive branch from `AUTHORING_BRANCH_PATTERN` at the proven base, or resume only its exact non-conflicting proven identity, before tracked authoring mutation. Use no task or refactor tag in commit or pull-request identity, and reject any other conflicting authoring branch or pull request.
2. Create the ignored `AUTHORING_RECOVERY_RECORD` for remote-operation intent and rehydration only. It is not an implementation scratchpad and cannot contain task status, `Pass`, closeout, archive, dependency, or completion state. Preserve it and the exact branch on failure or ambiguity.
3. Limit the change to the eligible authoring surfaces. Never edit `.harness/completed.md`; record its exact fetched-base blob identity. Task-authoring output may append only queued `Pass: false` tasks and advance only the applicable counter. It cannot claim, start, pass, close out, archive, satisfy, or complete a task it introduces.
4. Run the scoped registered sets selected for the changed surfaces, then `baseline` after the final content change. Inspect the complete diff, prove the recorded completed archive is byte-identical, commit with descriptive untagged identity, and bind `AUTHORING_CANDIDATE_SHA` to that immutable head.
5. Run `AGENT_REVIEW_PROCEDURE` against `AUTHORING_CANDIDATE_SHA`, plus each applicable registered security or visual review. Only the primary author repairs findings; every content change requires complete affected proof, `baseline`, a new candidate SHA, and fresh exact-SHA review.
6. Use `PUSH_COMMAND` and `PR_CREATE_OR_UPDATE_COMMAND` non-force, bind one exact configured-repository branch and pull request to the candidate, and use `REMOTE_OUTCOME_READBACK_PROCEDURE` for every remote operation. Durable PR evidence records explicit delivery and merge authority, exact-SHA proof and reviews, the read-only authoring/task exclusion case, and the unchanged completed-archive identity.
7. Run `EXACT_HEAD_CI_PROCEDURE` and `PRE_MERGE_BASE_REFRESH_PROCEDURE` for the unchanged authoring candidate. If the fetched base advanced, merge it into the published authoring branch with ordinary history-preserving Git, never rebase or force-push, then repeat local proof, completed-archive proof, exact-SHA review, non-force delivery, and exact-head CI. Authoring has no provisional transfer to reverse.
8. After the explicit manual or autonomous authority and all immediate base, head, protection, and no-bypass gates pass, use only `AUTHORING_MERGE_COMMAND`. Reconcile uncertainty through `REMOTE_OUTCOME_READBACK_PROCEDURE`, then require `AUTHORING_MERGE_READBACK_PROCEDURE`; command success alone is insufficient.
9. Reuse the exact identity, expected-old compare-and-delete, fast-forward synchronization, failure preservation, retry, and terminal-evidence mechanics in `POST_MERGE_CLEANUP_PROCEDURE`, substituting the authoring branch, candidate, PR, and merge identities and omitting every task-store, closeout, archive, and implementation-scratchpad action. Delete `AUTHORING_RECOVERY_RECORD` only after the terminal PR evidence is proven applied, both exact authoring refs are absent, local `BASE_BRANCH == origin/BASE_BRANCH`, and the worktree is clean.

The authoring delivery is complete only when authenticated readback proves the same PR merged into `BASE_BRANCH`, its exact merge OID is reachable and has first parent `EXPECTED_BASE_SHA`, its tree equals the reviewed authoring head tree, the subject remains descriptive and untagged, `.harness/completed.md` is byte-identical to the recorded base blob, the remote branch is absent, local base is synchronized and clean, and terminal cleanup evidence is applied. These facts are authoring-delivery evidence only and never satisfy task completion or dependency proof.
...
After task provisional closeout, or after final authoring-candidate review and delivery, and before either manual or autonomous merge:

1. Resolve exactly one pull request in `DevDoneDiff/cp` whose head repository is `DevDoneDiff/cp`, head ref is the exact task or authoring delivery branch, base ref is `BASE_BRANCH`, and `headRefOid` equals that remote branch SHA and `EXPECTED_HEAD_SHA`. A fork, duplicate, wrong base, mismatched remote branch, or moving PR head fails the identity gate.
...
4. Require the fetched `origin/BASE_BRANCH` SHA to be an ancestor of `EXPECTED_HEAD_SHA`, and reread the exact remote delivery branch and pull request to require both still equal `EXPECTED_HEAD_SHA`. A stale base, stale head, missing check, wrong identity, or bypass-capable merge path blocks merge.
5. If the fetched base differs from the base used for the current delivery or is not an ancestor of `EXPECTED_HEAD_SHA`, do not merge. Task delivery first performs and proves exact provisional reversal, then merges the fetched base into the retained task branch and restores active `working`/`Pass: false` state. Authoring delivery has no task-store reversal and merges the fetched base directly into the retained authoring branch. Both paths use ordinary history-preserving Git, reconcile only authorized content, never rebase or force-push, and never reuse stale validation or review. Treat the resulting committed head as a new candidate and repeat all applicable local proof, completed-archive proof, exact-SHA reviews, non-force push/readback, exact-head CI, and task closeout/inheritance only when applicable.
...
7. For a task in manual mode, preserve the proven closeout and stop for the user's merge decision. Once authorized, task delivery reruns steps 1 through 4 and uses only `MERGE_COMMAND`, with the exact closeout SHA and tagged subject. Authoring delivery instead follows its separately explicit manual or autonomous authority and uses only `AUTHORING_MERGE_COMMAND`, with the exact reviewed candidate and descriptive untagged subject. Both use squash mode, branch deletion, and no `--admin` or other bypass.
8. After the merge attempt has an unambiguous successful result, run the matching `MERGE_READBACK_PROCEDURE` or `AUTHORING_MERGE_READBACK_PROCEDURE`. Require the same PR to report `MERGED`, its reported merge commit OID to be reachable from the freshly fetched configured base, and that merge commit's first parent to equal `EXPECTED_BASE_SHA`. Then require the applicable tagged task/archive/active proof or descriptive authoring/tree/archive-immutability proof, plus exact remote branch absence. A different first parent proves a base race or bypass and blocks completion. Fast-forward local `BASE_BRANCH` to the exact fetched remote tip and require equal SHAs and a clean tree. Authoring evidence never advances the task queue; only the canonical task completion predicate may do so.

The pull request records the exact candidate and task closeout SHA when applicable, PR and branch identity, required check names/results/links, reviewed base SHA, `EXPECTED_BASE_SHA`, atomic up-to-date/no-bypass enforcement readback, any applicable base-advance reversal or authoring base merge and complete redelivery, guarded merge invocation, reported merge OID, exact merge first-parent equality, applicable archive-transition or archive-immutability and tree results, synchronized base SHA, and remote branch absence. Include at least one read-only base-advance procedure case and one completed-merge readback case. T-0032 owns failed or ambiguous remote-command classification and retry behavior.
...
| authoring guarded merge | `AUTHORING_MERGE_READBACK_PROCEDURE` proves the same PR merged, exact reported merge OID, `mergeCommit^ == EXPECTED_BASE_SHA`, configured-base reachability, descriptive untagged subject, reviewed-head tree equality, completed-archive immutability, and remote branch state | the same PR is still open with no merge OID, its exact head/base/remote-branch identities are unchanged, and fetched base history contains no intended authoring tree | applied: continue to authoring synchronization and cleanup without task-store mutation; not applied: rerun every exact-head/base/protection gate before the root intent's single retry |
...
- non-task authoring uses `AUTHORING_MERGE_COMMAND` and `AUTHORING_MERGE_READBACK_PROCEDURE` only through the activated authoring procedure; its descriptive identity and completion evidence never satisfy a task.
